...
# 1) Copia tus imágenes a la carpeta "fotos/" junto a este archivo.
...
def cargar_banco(subidas):
    """Junta las fotos de la carpeta fotos/ y las subidas desde el panel."""
    banco = {}

    if CARPETA_FOTOS.is_dir():
        for ruta in sorted(CARPETA_FOTOS.iterdir()):
...
if MODO_EDICION:
    ...
    with st.sidebar:
        ...
        st.caption(
            ...
            "definitivas: copia las fotos a la carpeta `fotos/` junto a "
            "`app.py` y pega este bloque en la sección "
            "FOTOS_FIJAS del código."
        )
    ...
    galeria_sel = list(FOTOS_FIJAS["galeria"])
    musica = None
