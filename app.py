...
FOTOS_FIJAS = {
    ...
    "historia": ["2.jpg", "3.jpg", "4.jpg", "5.jpg", "6.jpg","7.jpg"],       # 1 foto = arco; 2 a 4 fotos = collage
    "galeria": ["1.jpg","8.jpg"],        # lista de fotos, en el orden en que se mostrarán
...
html_block(
    ...
    .story-collage img:nth-child(1) {{ transform: rotate(-7deg); z-index: 2; }}
    .story-collage img:nth-child(2) {{ transform: rotate(5deg) translateY(-16px); z-index: 3; }}
    .story-collage img:nth-child(3) {{ transform: rotate(-4deg) translateY(10px); z-index: 1; }}
    .story-collage img:nth-child(4) {{ transform: rotate(8deg) translateY(-8px); z-index: 0; }}
...
if MODO_EDICION:
    ...
    with st.sidebar:
        ...
        historia_sel = st.multiselect(
            "Historia (1 foto = en arco · 2 a 4 = collage)",
            disponibles,
            default=historia_defecto,
            key="foto_historia"
        )

        if len(historia_sel) > 4:
            st.warning("Se usarán solo las primeras 4 fotos elegidas.")
            historia_sel = historia_sel[:4]
        ...
        if seleccion["vestuario"]:
            usadas.append(seleccion["vestuario"])
    ...
    seleccion = {
        ...
        for clave in ("portada", "vestuario", "cierre")
    }

    historia_sel = [n for n in FOTOS_FIJAS["historia"] if n in banco][:4]
...
if seleccion["vestuario"] in banco:
    ...
    uri_vestuario = a_data_uri(banco[seleccion["vestuario"]], 1400)

    html_block(
        f"""
        <section class="section section-olive" style="padding-top:0;">
            <div class="dress-photo-wrap">
                <img src="{uri_vestuario}"
                     style="width:100%; display:block;"
                     alt="Referencia de código de vestuario">
            </div>
        </section>
        """
    )
